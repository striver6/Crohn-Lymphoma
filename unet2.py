...
class myUnet(object):
    ...
    #     # model.load_weights("LYM_vfa/unet.hdf5")
    #     # model.load_weights("LYM_vfa/unet-2019-12-28-19-59-01-0.48.hdf5")
    #     # model.compile(optimizer = Adam (lr=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-08), loss = dice_coef_loss, metrics = [f1_score])
    #     model.compile(optimizer = Adam (lr=0.5*1e-6), loss = dice_coef_loss, metrics = [f1_score])
    #
    #     return model
    ...
    def train(self):
        ...
        model = self.get_unet()
        print("got unet")
        # model_checkpoint = ModelCheckpoint('LYM_vfa/unet-{}'.format(datetime.now().strftime('%Y-%m-%d-%H-%M'))+'-{epoch:02d}-{val_f1_score:.2f}.hdf5', monitor='loss',verbose=1, save_best_only=True)
